...
const STORAGE_KEY = 'alpha-learning-state-v4';
...
export async function hydrateStateFromSupabase(localState) {
  if (!supabase || typeof window === 'undefined' || !navigator.onLine || !localState?.profile?.id) return localState;

  try {
    const userId = localState.profile.id;

    const [{ data: user }, { data: sessions }, { data: conceptRows }, { data: streak }, { data: badges }] = await Promise.all([
      supabase.from('users').select('id,name,grade').eq('id', userId).single(),
      supabase.from('sessions').select('date,subject,duration_seconds,concepts_covered,score').eq('user_id', userId).order('date', { ascending: false }).limit(30),
      supabase.from('concept_progress').select('concept_id,subject,status,best_score,attempts,last_attempted').eq('user_id', userId),
      supabase.from('streaks').select('current_streak,longest_streak,last_session_date').eq('user_id', userId).single(),
    ...
    if (user) {
      ...
      next.profile.grade = user.grade || next.profile.grade;
    }
    ...
    if (Array.isArray(conceptRows)) {
      conceptRows.forEach((row) => {
        ...
        const node = next.subjects?.[subjectKey]?.conceptMap?.[conceptId];
        if (!node) return;

        node.status = row.status || node.status;
        node.bestScore = row.best_score ?? node.bestScore;
        node.attempts = row.attempts ?? node.attempts;
        node.lastPracticed = row.last_attempted || node.lastPracticed;
      });

      Object.keys(next.subjects || {}).forEach((subjectKey) => {
        next.subjects[subjectKey].mastery = recomputeSubjectMastery(next.subjects[subjectKey]);
      });
    }

    if (streak) {
      next.streak = streak.current_streak || 0;
      next.longestStreak = streak.longest_streak || 0;
      next.lastCompletionDate = streak.last_session_date || next.lastCompletionDate;
    }

    if (Array.isArray(badges)) {
      next.badges = badges.map((b) => b.badge_id).filter(Boolean);
    }

    if (Array.isArray(sessions) && sessions.length) {
      next.sessionLogs = sessions.map((s) => ({
        date: s.date,
        duration: Math.round((s.duration_seconds || 0) / 60),
        subject: s.subject,
        conceptId: s.concepts_covered?.[0] || null,
        conceptTitle: s.concepts_covered?.[0] || '',
        score: s.score || 0,
        mastered: (s.score || 0) >= 80,
      }));
      next.totalMinutes = next.sessionLogs.reduce((acc, log) => acc + (log.duration || 0), 0);
    }
